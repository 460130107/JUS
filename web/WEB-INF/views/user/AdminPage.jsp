...
<div class="ui-widget main-content">
    <div class="ui-widget-content" style="padding: 10px">
        ...
        <table border="1" width="100%">
            <tr><th>ID</th><th>用户名</th><th>呢称</th><th>邮箱</th><th width="80">状态</th><th width="120">操作</th></tr>
            <c:forEach items="${userList}" var="user">
                <tr><td>${user.id}</td><td>${user.name}</td><td>${user.nicename}</td><td>${user.email}</td><td>${user.status}</td><td style="text-align: center;"><a href="${pageContext.request.contextPath}/u/ep?uid=${user.id}" class="button">编辑属性</a></td></tr>
            </c:forEach>
        </table>
